<template>
  <a-card :bordered="false" class="search-form">
    <a-form :auto-form-create="(form) => {this.form = form;this.form.getFieldDecorator('owner',{initialValue: ['1', '2']})}">
      <form-row label="所属类目">
        <a-form-item>
          <tag-select>
...
<script lang="es6">
  const FormRow = load('@CMP/form/FormRow');
  const TagSelect = load('@CMP/tool/TagSelect');
  const TagSelectOption = () => TagSelect().then(cmp => cmp.Option());
