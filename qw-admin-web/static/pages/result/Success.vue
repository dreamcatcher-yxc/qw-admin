          ...
          <a-steps :current="1" progress-dot>
            <a-step title="创建项目">
              <a-step-item-group slot="description">
                <a-step-item title="曲丽丽" icon="dingding-o"></a-step-item>
                <a-step-item title="2016-12-12 12:32"></a-step-item>
              </a-step-item-group>
            </a-step>
            <a-step title="部门初审">
              <a-step-item-group slot="description">
                <a-step-item title="周毛毛" icon="dingding-o" :icon-style="{color: '#00A0E9'}"></a-step-item>
                <a-step-item title="催一下" :title-style="{color: '#00A0E9'}"></a-step-item>
              </a-step-item-group>
            </a-step>
...
<script lang="es6">
const Result = load('@CMP/result/Result');
const DetailList = load('@CMP/tool/DetailList');
const AStepItem = load('@CMP/tool/AStepItem');
const DetailListItem = () => DetailList().then(cmp => cmp.Item);
const AStepItemGroup = () => AStepItem().then(cmp => cmp.Group);
